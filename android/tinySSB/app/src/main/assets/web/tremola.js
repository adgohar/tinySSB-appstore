...
function appController() {
    //backend("apps:createApp Jedi:Survivor Star_Wars");
    //backend("apps:createApp Witcher3 The_Witcher");
    //backend("apps:writeToFeed da97f79165dd7e17a8122a064b3f4a86d49408dd08fd978b391d8fb2cd4b7f87")
    //backend("apps:readFromFeed f369faaf7803fa2141a33d59402ff444bc982f3e3e2659f5d61aeb00b4436152 4")

    //backend("apps:insertAssetIntoFeed fa4614124e0ee8a25006fbf9c8dc02782d066c79fcaee1a4593c075c51b5c2e9 Zuerich.png")
    //backend("apps:insertAssetIntoFeed fa4614124e0ee8a25006fbf9c8dc02782d066c79fcaee1a4593c075c51b5c2e9 Basel.png")
    //backend("apps:readFromFeed fa4614124e0ee8a25006fbf9c8dc02782d066c79fcaee1a4593c075c51b5c2e9 1")
    //backend("apps:readFromFeed fa4614124e0ee8a25006fbf9c8dc02782d066c79fcaee1a4593c075c51b5c2e9 2")

    //backend("apps:insertReleaseIntoFeed fa4614124e0ee8a25006fbf9c8dc02782d066c79fcaee1a4593c075c51b5c2e9 1.7 this_is_my_second_release")
    backend("apps:readFromFeed fa4614124e0ee8a25006fbf9c8dc02782d066c79fcaee1a4593c075c51b5c2e9 2")
...
function showAppsConfig() {
    setScenario("appsConfig");
    backend("apps:listAppFeeds");
}

function listAppFeeds(appFeedsString) {
    while (document.getElementById("lst:appsConfig").firstChild) {
        document.getElementById("lst:appsConfig").firstChild.remove()
    }
    let appFeeds = parseStringToDictionary(appFeedsString);
    for (let app in appFeeds) {

        const button = document.createElement("button");

        button.textContent = appFeeds[app];

        button.id = appName + "Btn";

        button.onclick = function() { getAppReleases(app); };

        document.getElementById("lst:appsConfig").appendChild(button);
    }
}

function getAppReleases(appId) {
    setScenario("appSettings");
    backend("apps:listAppReleases " + appId);
}

function listAppReleases(releaseListString) {
    const appSettings = document.getElementById("lst:appSettings");
    while (appSettings.firstChild) {
        appSettings.firstChild.remove();
    }
    let releaseList = parseStringToDictionary(releaseListString);
     for (let release in releaseList) {
        const releaseDiv = document.createElement("div");
        let releaseElements = release.split(";");
        const releaseInfo = document.createTextNode(`Release: ${releaseElements[1]}, Comment: ${releaseList[release]}`);
        releaseDiv.appendChild(releaseInfo);

        const button = document.createElement("button");
        button.textContent = "Download";
        // Use let here to create a new binding for each iteration of the loop
        let buttonId = release + "Btn";
        button.id = buttonId;
        button.onclick = function() { activateRelease(releaseElements[0], releaseElements[1]); };

        releaseDiv.appendChild(button);

        appSettings.appendChild(releaseDiv);
    }
}

function activateRelease(fid, version) {
    backend("apps:activateRelease " + fid + " " + version)
}

function parseStringToDictionary(inputString) {
    let cleanedString = inputString.replace(/{|}/g, "").trim();
    let entries = cleanedString.split(', ');
    let dictionary = {};
