...
class NodeGoset(val context: MainActivity, val repo: Repo) {
    val NODE_ROUND_LEN = 5000L
    var log_offs = 0
    var chunk_offs = 0
    val goset = repo.goset
    ...
    fun incoming_want_request(buf: ByteArray, aux: ByteArray?, sender: String?) {
        ...
        for (i in 1..lst.lastIndex) {
            ...
            var seq: Int
            try {
                val ndx = (offs + i - 1) % goset.keys.size
                fid = goset.keys[ndx]
                seq = lst[i].getInt()
                v += " $ndx.${seq}"
                vector[ndx] = seq
            ...
            while (credit > 0) {
                val pkt = repo.feed_read_pkt(fid, seq)
    ...
    fun incoming_chunk_request(buf: ByteArray, aux: ByteArray?) {
        ...
        for (e in vect.getBipfList()) {
            ...
            while (cnr <= maxChunks && credit-- > 0) {
                val chunk = repo.feed_read_chunk(fid, seq, cnr)
                if (chunk == null) break;
    ...
    fun beacon() { // called in regular intervals
        ...
        val want_buf = repo.mk_want_vect()
        if (want_buf != null)
            context.tinyIO.enqueue(want_buf, context.tinyDemux.want_dmx)

        val chnk_buf = repo.mk_chnk_vect()
        if (chnk_buf != null)
    ...
    fun incoming_chunk(buf: ByteArray, fid: ByteArray?, seq: Int) {
        ...
        if (fid == null) return
        if (buf.size != TINYSSB_PKT_LEN) return
        repo.sidechain_append(buf, fid, seq)
    }
